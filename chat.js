...
const  arquivos = await leArquivos(["./database/InovaçãonaEconsiste.txt" , "./database/SuportenaEconsiste.txt", "./database/ConsultorianaEconsiste.txt"]);

const documentos = await incorporarDocumentos(arquivos);

export async function InputChat(mensagem) {
